...
const { maybeBuildPengKaipingV01Preview } = require('./p0-expression-assets');

const VERSION = 'report-os-v1-p0-rules-v0.2';
const MAX_BODY_BYTES = 512 * 1024;
const MAX_IMAGE_SIZE_BYTES = 10 * 1024 * 1024;
const SUPPORTED_IMAGE_MIME_TYPES = ['image/jpeg', 'image/png', 'image/webp'];
const MULTIPART_UNSUPPORTED_MESSAGE = 'report-upload-p0 当前只支持 JSON dry-run 图片元信息，不支持 multipart 文件上传；真实图片识别请走 /api/extract-fp。';
...
function contentType(req) {
  return lower(req && req.headers && req.headers['content-type']);
}
...
function base64PayloadLength(value) {
  ...
  return commaIndex >= 0 ? raw.slice(commaIndex + 1).length : raw.length;
}

function buildImageDryRun(payload) {
  const source = payload && typeof payload === 'object'
    ? (payload.imageInput && typeof payload.imageInput === 'object' ? payload.imageInput : payload)
    : {};
  const imageBase64 = source.imageBase64;
  const hasImage = !!(source.fileName || source.mimeType || source.sizeBytes || imageBase64);
  ...
  const fileName = text(source.fileName || source.name);
  const mimeType = text(source.mimeType || source.imageMimeType || source.type);
  const sizeBytes = numberOrNull(source.sizeBytes || source.fileSize || source.size);
  const base64Length = base64PayloadLength(imageBase64);
  const warnings = [];

  if (mimeType && !SUPPORTED_IMAGE_MIME_TYPES.includes(lower(mimeType))) {
    warnings.push({
      code: 'unsupported_mime_type',
      message: '当前 dry-run 只建议接收 image/jpeg、image/png、image/webp；本轮不会崩溃，也不会调用真实识别。',
    });
  }
  if (sizeBytes !== null && sizeBytes > MAX_IMAGE_SIZE_BYTES) {
    warnings.push({
      code: 'image_size_over_10mb',
      message: '图片大小超过 10MB；dry-run 仅记录元信息，真实识别前建议压缩。',
    });
  }

  const safeImageInput = {
    received: true,
    fileName,
    mimeType,
    sizeBytes,
    base64Length,
    hasImageBase64: base64Length > 0,
  };
...
function normalizePayload(payload) {
  ...
  const customUserQuestion = text(payload.customUserQuestion);
  ...
  const parseResult = parseReportText([reportText, customUserQuestion].filter(Boolean).join('\n'));
  return {
    reportText,
    customUserQuestion,
...
function assessContextRisks(ctx) {
  ...
  if (!ctx.consentConfirmed && needsConsent(ctx)) {
    pushRisk(risks, 'R2', 'third_party_without_confirmed_consent', '他人或未成年人报告授权不清，至少 R2。');
  }
...
function needsConsent(ctx) {
  ...
  const subject = lower(ctx.reportSubject);
  const relation = lower(ctx.subjectRelation);
  return includesAny(`${identity} ${subject} ${relation}`, [
    '孩子', '未成年', '学生', '他人', '客户', '伴侣', '合伙人', '团队', '班级', '企业', '学校',
    'child', 'minor', 'student', 'candidate', 'client', 'partner', 'team', 'class', 'enterprise', 'school',
    'parent_child', 'teacher_student', 'other',
  ]);
}
...
function safeMainQuestion(ctx, riskLevel) {
  const source = `${ctx.userIntent} ${ctx.customUserQuestion || ''} ${ctx.reportType} ${ctx.reportSubject} ${ctx.subjectRelation}`;
...
function hasSectionContent(section) {
  return !!(text(section && section.body) || (Array.isArray(section && section.bullets) && section.bullets.some(Boolean)));
}

function defaultThreePartSections(ctx) {
  const isChild = includesAny(`${ctx.reportSubject} ${ctx.subjectRelation} ${ctx.userIntent} ${ctx.customUserQuestion || ''}`, ['孩子', '学生', 'child', 'student', 'parent_child']);
  return [
    buildSection(
      'why',
      '为什么会这样',
      isChild
        ? '可以先把这个表现理解为孩子在任务启动、坚持或情绪调节上需要更多支持，而不是性格定论。'
        : '可以先把这个倾向理解为你在信息处理、节奏安排或压力场景中的一种线索，而不是固定结论。',
      ['需要结合具体场景观察', '如果和现实不贴近，可以作为校正线索'],
      'why'
    ),
    buildSection(
      'how',
      '怎么应对',
      isChild
        ? '先把要求缩小成第一个可执行动作，再观察孩子在哪一步更容易卡住。'
        : '先选一个最近真实发生的小场景，用更小的行动去验证报告是否贴近。',
      isChild
        ? ['先观察是启动困难、坚持困难，还是收尾困难', '少用连续催促，多给一个明确可做的第一步']
        : ['先观察它在哪些场景反复出现', '把问题改成一个可尝试的小动作'],
      'how'
    ),
    buildSection(
      'future',
      '未来可期',
      '这不是定论，而是一个可以被观察、校正和发展的方向。合适的环境和支持方式会让倾向更容易转成可用能力。',
      ['后续可以继续观察变化', '重要决定仍建议结合现实反馈和人工解读'],
      'future'
    ),
  ];
}

function normalizeThreePartSections(sections, ctx) {
  const defaults = defaultThreePartSections(ctx);
  const source = Array.isArray(sections) ? sections : [];
  const findSection = keywords => source.find(section => includesAny(`${section.heading || ''} ${section.id || ''} ${section.sectionType || ''}`, keywords));
  const picked = [
    findSection(['为什么会这样', 'why']) || source[0],
    findSection(['怎么应对', 'how', '建议', '沟通', '观察']) || source[1],
    findSection(['未来可期', 'future', '下一步', '人工']) || source[2],
  ];
  return defaults.map((fallback, index) => {
    const section = picked[index] || {};
    const next = {
      ...fallback,
      body: text(section.body || section.content) || fallback.body,
      bullets: Array.isArray(section.bullets) && section.bullets.length ? section.bullets.filter(Boolean) : fallback.bullets,
    };
    return hasSectionContent(next) ? next : fallback;
  });
}
...
function safeDiscussionDirections(ctx, blockedReasons) {
  ...
  if (includesAny(source, ['medical', 'psychological', 'diagnosis', '心理', '医学', '诊断', '疾病'])) {
    return ['先记录具体表现出现了多久、在什么场景更明显', '看它是否已经影响睡眠、学习、情绪或日常生活', '如果影响持续存在，建议寻求专业支持，而不是只用报告解释'];
  }
  if (includesAny(source, ['enterprise', 'team', 'candidate', '招聘', '录用', '淘汰', '企业', '团队'])) {
    return ['在明确授权和脱敏前提下，只讨论沟通与协作支持方式', '把问题改成“怎样帮助这个人在团队中更好协作”', '由人工确认用途边界，避免把报告用于人事筛选或去留决定'];
  }
  if (includesAny(source, ['school', 'class', 'student', '学校', '班级', '分层', '分班'])) {
    return ['把问题改成学习方式、任务支持和沟通节奏', '先确认监护授权、使用目的和资料可见范围', '用于帮助学生获得支持，而不是做学生分组或处置决定'];
  }
  if (includesAny(source, ['relationship', 'partner', '关系', '伴侣', '婚姻', '合伙'])) {
    return ['先整理最近一次具体冲突，而不是直接判断关系走向', '看双方真正卡住的是需求、节奏、表达，还是边界', '下一次沟通可以先确认“我需要什么”和“对方听到了什么”'];
  }
  if (includesAny(source, ['guarantee', 'destiny', 'mysticism', '升学', '成功', '命运'])) {
    return ['把问题放回现实里的学习、选择和资源条件', '用短期可验证的观察点替代结果承诺', '把不贴近的地方当作校正线索，而不是解释成命运'];
  }
...
function buildQuickVisibleSections(ctx, quickReading, outputType) {
  ...
  const referencePoints = isSafe ? [
    '先看孩子是在“开始任务”困难、坚持过程中困难，还是收尾时更容易卡住。',
    '观察情绪反应通常出现在被催促、任务太大、时间太紧，还是身体已经疲惫的时候。',
    '把报告里的倾向当作提醒，再回到年龄阶段、家庭节奏和学校任务中校正。',
    '如果报告内容和真实表现不贴近，就把不贴近的地方作为继续观察的线索。',
  ] : [
    '你处理信息时，可能更需要清楚的节奏、边界或选择空间。',
    '压力较大时，可以观察自己更容易卡在启动、坚持、决策，还是收尾阶段。',
    '沟通中可以留意：具体场景、明确要求和可执行步骤，是否更容易让你进入状态。',
    '如果有不贴近的部分，先不要否定自己，也不要硬套报告，可以把它作为校正线索。',
  ];
  const suggestions = isSafe ? [
    '先把要求缩小成第一个可执行动作，例如“先写第一题的题号”。',
    '少用连续催促，多确认孩子是不会开始、太累，还是不知道怎么收尾。',
    '连续观察 1-2 周，看睡眠、任务量、家庭节奏和学校压力是否有变化。',
  ] : [
    '先选一个最近真实发生的小场景，观察 1-2 周。',
    '把“我是不是这样的人”改成“我在什么情境下更容易这样反应”。',
    '遇到重大职业、关系或学习选择时，把报告当参考，不直接当决定依据。',
  ];
  ...
  return [
    buildSection(
      ...
      isSafe ? ['先看行为出现的场景', '先看孩子需要哪种支持', '不急着给孩子贴标签'] : referencePoints.slice(0, 3),
      'why'
    ),
    buildSection(
      'how',
      '怎么应对',
      isSafe ? '先把行为翻译成需要支持的地方，再决定怎么回应。' : '先用小范围观察替代直接判断。',
      suggestions,
      'how'
    ),
    buildSection(
      ...
      isSafe ? ['观察 1-2 周的具体变化', '如果持续影响学习、睡眠、情绪或亲子关系，建议人工一起看'] : ['把不贴近的地方作为校正线索', '重大选择仍建议结合现实反馈和人工解读'],
      'future'
    ),
  ];
}
...
function buildClarificationVisibleSections(ctx, clarificationQuestions) {
  const defaultQuestions = [
    '这份报告是看本人、孩子，还是他人？',
    '你最想解决的是学习、沟通、行为、职业，还是关系问题？',
    '你能否补充报告中最关键的一段文字或指标？',
  ];
  const questions = (clarificationQuestions.length ? clarificationQuestions : defaultQuestions).slice(0, 3);
  return [
    buildSection(
      'why',
      '为什么会这样',
      '为了避免把报告讲偏，我想先确认几个关键信息。',
      ['不硬凑结论', '不补写缺失资料', '先确认对象、目的和授权'],
      'why'
    ),
    buildSection(
      'how',
    ...
    buildSection(
      ...
      ['资料越清楚，输出越贴近真实问题', '仍会保持安全边界，不做诊断、预测或重大决定'],
      'future'
    ),
  ];
}

function fallbackScenario(ctx, blockedReasons) {
  const blockedCodes = blockedReasons.map(reason => reason.code).join(' ');
  const source = `${ctx.userIntent} ${ctx.reportType} ${ctx.reportSubject} ${ctx.subjectRelation} ${blockedCodes}`;
  if (includesAny(source, ['medical', 'psychological', 'diagnosis', '心理', '医学', '诊断', '疾病'])) return 'diagnosis';
  if (includesAny(source, ['relationship', 'partner', '关系', '伴侣', '婚姻', '合伙'])) return 'relationship';
  if (includesAny(source, ['enterprise', 'team', 'candidate', '招聘', '录用', '淘汰', '企业', '团队'])) return 'enterprise';
  if (includesAny(source, ['school', 'class', 'student', '学校', '班级', '分层', '分班'])) return 'school';
  if (includesAny(source, ['destiny', 'mysticism', '命运', '注定', '玄学'])) return 'mysticism';
  if (includesAny(source, ['education', 'career', 'guarantee', '升学', '职业', '成功'])) return 'education';
  return 'general';
}

function fallbackCopy(ctx, blockedReasons) {
  const scenario = fallbackScenario(ctx, blockedReasons);
  const copies = {
    diagnosis: {
      title: '这个问题不适合直接用报告判断',
      subtitle: '报告不能判断心理、医学或发展问题，可以先整理具体表现和影响范围。',
      explanation: '这类问题更适合看真实表现、持续时间和影响范围，必要时寻求专业支持。',
      bullets: ['先记录表现出现多久', '看是否影响睡眠、学习、情绪或生活', '不要只用报告解释专业问题'],
      next: '如果影响持续存在，建议带着具体记录寻求专业支持或人工安全解读。',
    },
    relationship: {
      title: '关系问题不适合自动下结论',
      subtitle: '报告不能判断关系走向，也不能替你决定谁对谁错。',
      explanation: '关系问题更适合回到一个具体冲突场景，看双方需求、表达方式和边界。',
      bullets: ['先描述最近一次具体冲突', '看双方真正想表达的需要', '把沟通方式作为观察重点'],
      next: '建议先整理具体场景，再由人工在安全边界内一起看。',
    },
    enterprise: {
      title: '这份报告不能用于人事决定',
      subtitle: '如果有授权，它最多用于理解协作和沟通支持方式。',
      explanation: '企业场景需要明确授权、脱敏和用途边界，不能把报告作为人事去留依据。',
      bullets: ['只讨论协作偏好和支持方式', '不做人事去留决定', '敏感用途建议人工复核'],
      next: '可以把问题改成“怎样支持协作更顺畅”，再由人工确认用途边界。',
    },
    school: {
      title: '这份报告不能用于学生分组决定',
      subtitle: '学校场景更适合讨论学习支持策略，而不是做学生处置或筛查。',
      explanation: '涉及学生时，需要先确认监护授权、用途和可见范围，默认更保守。',
      bullets: ['只讨论学习方式和支持策略', '不做学生筛查、排名或处置决定', '保护未成年人标签和隐私'],
      next: '建议由人工一起确认授权、用途和支持策略，再决定如何沟通。',
    },
    education: {
      title: '报告不能承诺升学或职业结果',
      subtitle: '它可以帮助观察学习方式、压力场景和支持策略，但不能承诺路线和结果。',
      explanation: '学习和职业选择需要结合兴趣、资源、环境、机会和持续反馈。',
      bullets: ['先观察学习方式和压力反应', '用小范围尝试验证兴趣和能力', '不要把报告当成结果承诺'],
      next: '可以先整理真实学习场景，再人工一起看哪些支持策略更合适。',
    },
    mysticism: {
      title: '报告不能判断命运',
      subtitle: '它更适合帮助你观察偏好、沟通节奏和现实选择。',
      explanation: '命运、天命或注定关系不适合用报告解释。更稳妥的做法是回到现实行为和选择。',
      bullets: ['观察现实偏好和行为模式', '用具体场景验证是否贴近', '不把报告解释成命定结论'],
      next: '建议把问题改成一个现实选择或沟通场景，再继续解读。',
    },
    general: {
      title: '这个问题更适合先人工确认',
      subtitle: '当前问题涉及较重要的判断，先整理具体场景会更稳。',
      explanation: '直接给结论容易误导，先把资料、授权、真实场景和想解决的问题说清楚。',
      bullets: ['先说明你真正想解决的问题', '确认资料是否清晰且已授权', '复杂场景建议人工一起看'],
      next: '请先补充具体场景，或转人工做安全解读。',
    },
  };
...
function buildFallbackVisibleSections(ctx, blockedReasons, fallbackMessage, humanReviewQueueDryRun) {
  const copy = fallbackCopy(ctx, blockedReasons);
  return [
    buildSection(
      'why',
      '为什么会这样',
    ...
    buildSection(
      ...
      '未来可期',
      copy.next,
      ['先说明你真正想解决的问题', '准备清晰且已授权的资料', '后续可以在安全边界内继续解读'],
      'future'
    ),
  ];
}
...
function buildUserVisibleOutput({
  ...
  if (outputType === 'quick_reading_output') {
    title = '先快速读懂这份报告';
    subtitle = '这不是对你的定论，而是帮你找到几个值得观察的线索。';
    ...
    title = '先看见孩子行为背后的需要';
    subtitle = '孩子的表现不能直接等同于性格，我们先从场景和支持方式看起。';
    sections = buildQuickVisibleSections(ctx, quickReading, outputType);
    cta = {
      type: 'human_review',
      label: '人工一起看孩子的具体行为',
    ...
    cta = {
      ...
      label: humanReview && humanReview.recommended ? '转人工安全解读' : '先整理具体场景',
      message: humanReview && humanReview.recommended
        ? '这个问题更适合由人工结合完整资料和安全边界一起看。'
        : '请先补充你真正想解决的具体场景。',
    };
  }
  ...
  return {
    ...
    title,
    subtitle,
    sections: normalizeThreePartSections(sections, ctx),
    cta,
    safetyNotice: unique([
    ...
    meta: {
      ...
      customUserQuestionProvided: !!ctx.customUserQuestion,
    },
  };
}
...
function buildResponse(payload) {
  ...
  const imageDryRunResult = buildImageDryRun(payload);
  ...
  const userVisibleOutput = buildUserVisibleOutput({
    ...
    userIdentity: ctx.userIdentity,
    userIntent: ctx.userIntent,
    customUserQuestion: ctx.customUserQuestion,
    reportType: ctx.reportType,
    reportSubject: ctx.reportSubject,
    subjectAge: ctx.subjectAge,
    subjectRelation: ctx.subjectRelation,
  ...
  const pengkaipingV01 = maybeBuildPengKaipingV01Preview({
    expressionId: payload && payload.pengkaipingExpressionId,
  });
  const response = {
    ok: true,
    stage,
    ...
    humanReviewQueueDryRun,
    userVisibleOutput,
    imageInput: imageDryRunResult.imageInput,
    imageDryRun: imageDryRunResult.imageDryRun,
    safetyNotes: SAFETY_NOTES,
  };

  if (pengkaipingV01) {
    response.pengkaipingV01 = pengkaipingV01;
  }
...
module.exports = async function handler(req, res) {
  ...
  if (contentType(req).includes('multipart/form-data')) {
    return res.status(415).json({
      ok: false,
      stage: 'read_json',
      error: MULTIPART_UNSUPPORTED_MESSAGE,
      imageDryRun: {
        received: false,
        actualRecognitionCalled: false,
        recognitionStatus: 'multipart_not_supported',
      },
    });
  }
