...
function buildQuickVisibleSections(ctx, quickReading, outputType) {
  ...
  return [
    buildSection(
      'safety_intro',
      isSafe ? '先看见行为背后的需要' : '先把报告放在合适的位置',
      isSafe
        ? '孩子的表现不能直接等同于性格。我们先看这个行为在什么场景出现、它可能在表达什么需要。'
        : '这份报告可以先看作一份“理解自己偏好和沟通方式的参考”，不是对你的定论。',
      isSafe ? ['先看场景，不贴标签', '先看支持方式，不追责父母', '先看当下表现，不判断未来'] : ['不做诊断', '不预测未来', '不替代现实观察'],
      'safety_intro'
    ),
    buildSection(
      'report_overview',
      ...
      isSafe ? ['哪些表现可以先观察', '哪些支持方式可能更有效', '哪些结论暂时不适合下'] : ['可以参考的倾向先解释', '不清楚的信息不补写', '重大选择不直接下结论'],
      'report_overview'
    ),
    buildSection(
      'main_question',
    ...
    buildSection(
      'reference_points',
      '可以参考的理解点',
      isSafe ? '可以先从下面几个方向观察孩子，而不是急着判断孩子“是什么样的人”。' : '可以先从下面几个方向观察自己，而不是急着把报告当成结论。',
    ...
    buildSection(
      ...
      isSafe ? ['看这个表现出现在哪些具体场景', '看它持续多久、影响学习或情绪到什么程度', '看近期作息、任务量或亲子互动是否有变化'] : ['看这个反应常出现在哪类任务里', '看它持续多久、影响多大', '看近期环境或角色是否有变化'],
      'environment_observation'
    ),
    buildSection(
...
function buildClarificationVisibleSections(ctx, clarificationQuestions) {
  ...
  return [
    ...
    buildSection(
      ...
      ['涉及孩子会更保守', '涉及他人资料要先确认授权', '涉及重大判断会建议人工一起看'],
      'report_overview'
    ),
    buildSection(
      'communication_suggestions',
...
function buildFallbackVisibleSections(ctx, blockedReasons, fallbackMessage, humanReviewQueueDryRun) {
  ...
  return [
    ...
    buildSection(
      ...
      ['只看可参考的倾向', '不把单一报告当成决定依据', '重要问题交给人工复核更稳妥'],
      'report_overview'
    ),
    buildSection(
      'environment_observation',
    ...
    buildSection(
      ...
      ['不做医学/心理判断', '不做关系或合作结论', '不做机构处置或结果承诺类判断'],
      'no_direct_conclusion'
    ),
    buildSection(
      'human_review_suggestion',
      '建议下一步',
      copy.next,
      ['先说明你真正想解决的问题', '准备清晰且已授权的资料', '必要时寻求专业支持'],
      'human_review_suggestion'
    ),
  ];
}
