...
function searchReportKnowledge(queryText, options = {}) {
  ...
  return index.entries
    ...
    .map(result => ({
      ...
      retrievalKeywords: result.entry.retrievalKeywords || [],
      score: result.score,
    }));
}
...
function mapRetrievalHits(queryText, options = {}) {
  ...
  return results.map(result => ({
    ...
    retrievalKeywords: result.retrievalKeywords || [],
    score: result.score,
...
function issueFocusTerms(issue) {
  const text = String(issue || '');
  const groups = [
    ['文理', '选科', '天赋更偏'],
    ['偏科', '学习方法', '阅读', '记忆', '科目'],
    ['升学', '名校', '志愿', '专业'],
    ['父母', '三观', '亲子', '顶嘴', '叛逆', '沟通', '相处'],
    ['伴侣', '婚姻', '分手', '关系', '在一起'],
    ['拖拉', '磨蹭', '作业', '主动', '内驱', '手机', '游戏'],
    ['焦虑', '情绪', '生气', '输不起', '压力', '崩溃'],
    ['朋友', '人际', '同学', '老师', '课堂', '社交'],
    ['职业', '工作', '转型', '事业', '创业'],
  ];
...
function titleSimilarity(leftTitle, rightTitle) {
  const grams = value => {
    const text = normalizeText(value).replace(/[^一-龥a-z0-9]/g, '');
    const set = new Set();
    for (let i = 0; i < text.length - 1; i += 1) set.add(text.slice(i, i + 2));
    return set;
  };
  const left = grams(leftTitle);
  const right = grams(rightTitle);
...
function selectIssueHits(issue, hits, maxItems) {
  const focusTerms = issueFocusTerms(issue);
  const relevant = focusTerms.length
    ? hits.filter(hit => {
        const searchable = [hit.title, ...(hit.scenarios || []), ...(hit.retrievalKeywords || [])].join(' ');
        return focusTerms.some(term => searchable.includes(term));
      })
    : hits;
  const candidates = relevant.length ? relevant : hits;
  const selected = [];
  for (const hit of candidates) {
    if (selected.some(existing => titleSimilarity(existing.title, hit.title) >= 0.58)) continue;
    selected.push(hit);
...
function buildReportKnowledgePromptContext(input = {}, options = {}) {
  ...
  const autoHits = uniqueDryRunHits(
    dryRun,
    (hit, stage) => hit.canUseForAutoOutput && stage.stage !== 'user_questions',
  ...
  const issues = [
    ...(input.selectedIssues || input.userQuestions || []),
    ...(input.customUserQuestion ? [input.customUserQuestion] : []),
  ].map(item => String(item || '').trim()).filter(Boolean);
  const issueKnowledgeContexts = {};
  for (const issue of [...new Set(issues)]) {
    ...
    const hits = mapRetrievalHits(query, {
      index: options.index,
      topK: options.issueTopK || 3,
      allowedStatuses: ['auto_safe', 'rewrite_required', 'human_only', 'blocked'],
    });
    const usableHits = selectIssueHits(
      issue,
      hits.filter(hit => hit.canUseForAutoOutput),
      options.maxItemsPerIssue || 2
    );
    const riskHits = selectIssueHits(issue, hits.filter(hit => hit.isGuardrailOnly), 2);
    issueKnowledgeContexts[issue] = {
      hitIds: hits.map(hit => hit.id),
      knowledgeBlock: buildReportGroundingBlock(usableHits, { maxItems: options.maxItemsPerIssue || 2 }),
      riskBlock: buildRiskGroundingBlock(riskHits, { maxItems: 2 }),
    };
  }
  ...
  return {
    ...
    retrievalSummary: {
      ...
      issueHitIds: Object.fromEntries(
        Object.entries(issueKnowledgeContexts).map(([issue, context]) => [issue, context.hitIds])
      ),
    },
  };
}
