...
;(function (root, factory) {
  ...
  function classifyBySymbolsOnly(thumbs, allSymbols) {
    ...
    // ── RULE 2: 十指全斗 → 超级认知型 ──────────────────────────────
    // 完整版: 主导拇指(TRC高者)决定 A/B/C。
    // 速测降级: 两拇指符号相同才能判子型; 不同则无法定主导→输出大类+提示。
    if (totalW >= 10) {
      if (rt === lt) {
        // 两拇指同符号 → 可判 A/B/C
        if (_isDouble(rt))  return { mainType: '超级认知B · 双核整合', key: 'super_w_b', tagline: '领袖气质 · 全局整合 · 多维并行 · 跨界高手' };
        if (_isPeacock(rt)) return { mainType: '超级认知C · 完美特质', key: 'super_w_c', tagline: '领袖气质 · 极致审美 · 完美追求 · 卓越标准' };
        return { mainType: '超级认知A · 螺旋领袖', key: 'super_w_a', tagline: '天生领袖 · 主见极强 · 目标必达 · 王者之气' };
      }
      // 两拇指符号不同 → 无法确定主导拇指 → 降级
      return {
        mainType: '超级认知型', key: 'super_w_a',
        tagline: '天生领袖 · 极强主见 · 超级认知潜力',
        upgradeHint: '十指全斗、两拇指子型不同，A/B/C精确分型需上传总表',
      };
    }
    ...
    // ══ 前四条大类型（逆思/超级认知/全弧开放/超级模仿）均未命中 ══
    // 以下只在上述全部排除后执行，进入"拇指判定区":
    //   食指弧修饰 → 主导拇指标签兼开放型
    //   两拇指同标签 → 单一型
    //   两拇指不同家族 → 兼型(高家族在前: 斗>箕>弧)
    //   两拇指同家族不同子型 → 速测无TRC定不了主导拇指 → 降级
    ...
    const ltLbl = _thumbLbl(lt), rtLbl = _thumbLbl(rt);
    const ltFam = FAM[ltLbl] || '斗', rtFam = FAM[rtLbl] || '斗';

    // ── RULE 3b: 食指有弧 → 主导拇指标签兼开放型 ───────────────────
    ...
    if (hasIndexArc) {
      ...
      if (ltFam !== rtFam) {
        // 不同家族 → 高家族即为主导
        domLbl = FAMRANK[ltFam] <= FAMRANK[rtFam] ? ltLbl : rtLbl;
      } else if (ltLbl === rtLbl) {
        domLbl = ltLbl;  // 同标签直接用
      } else {
        // 同斗家族不同子型 → 无法定主导 → 降级
        return {
          mainType: '认知兼开放型', key: 'combo_open',
          tagline: '主见基调 · 开放加持 · 吸收力强',
          upgradeHint: '两拇指同为斗纹但子型不同，精确标签需上传总表',
        };
      }
      if (domLbl !== '开放')
        return { mainType: `${domLbl}兼开放型`, key: 'combo_open', tagline: `${domLbl}主导 · 开放加持 · 吸收力强` };
    }
    ...
    if (ltLbl === rtLbl) return SINGLE[ltLbl] || SINGLE['认知'];

    // ── RULE 5续: 同家族(两斗)不同子型 → 不兼，取主导拇指单一型 ────
    // 速测无TRC → 无法定主导 → 降级提示
    if (ltFam === rtFam) {
      return {
        mainType: '认知型', key: 'w',
        tagline: '独立思考 · 主见权威 · 探索求知',
        upgradeHint: '两拇指同为斗纹但子型不同，精确分型（认知/整合/完美）需上传总表',
      };
    }

    // ── RULE 6: 不同家族 → 兼型，高家族在前(斗>箕>弧) ─────────────
