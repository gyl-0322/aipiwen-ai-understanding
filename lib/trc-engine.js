...
;(function (root, factory) {
  ...
  function classifyBySymbolsOnly(thumbs, allSymbols) {
    ...
    // 同斗家族子型优先级: 整合 > 完美 > 认知 (与计算引擎/00 §三一致)
    // 速测无TRC无法定主导拇指，用此优先级代理（给出确定答案）
    function _wDominant(lbl1, lbl2) {
      const P = { '整合':0, '完美':1, '认知':2 };
      return (P[lbl1] ?? 2) <= (P[lbl2] ?? 2) ? lbl1 : lbl2;
    }

    // ── RULE 1: 任意手指有反箕(Rl) → 逆思型 ────────────────────────
    ...
    // 完整版用主导拇指TRC定子型；速测用 完美>整合>认知 优先级代理
    if (totalW >= 10) {
      const dom = _wDominant(ltLbl, rtLbl);
      if (dom === '完美') return { mainType: '超级认知C · 完美特质', key: 'super_w_c', tagline: '领袖气质 · 极致审美 · 完美追求 · 卓越标准' };
      if (dom === '整合') return { mainType: '超级认知B · 双核整合', key: 'super_w_b', tagline: '领袖气质 · 全局整合 · 多维并行 · 跨界高手' };
    ...
    // ══ 前四条大类型均未命中 → 拇指判定区 ══════════════════════════
    // 规则: 食指弧修饰 / 两拇同标签→单一型 / 不同家族→兼型 / 同斗不同子型→优先级取单一型
    ...
    if (hasIndexArc) {
      ...
      if (ltFam !== rtFam) {
        ...
        domLbl = _wDominant(ltLbl, rtLbl);  // 同斗不同子型 → 优先级代理
      }
      if (domLbl !== '开放')
    ...
    // ── RULE 5续: 同斗家族不同子型 → 不兼，优先级(完美>整合>认知)取单一型 ─
    if (ltFam === rtFam) {
      return SINGLE[_wDominant(ltLbl, rtLbl)] || SINGLE['认知'];
    }
