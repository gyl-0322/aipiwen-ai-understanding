...
;(function (root, factory) {
  ...
  const SUBPRI   = { '完美':0, '整合':1, '认知':2 };   // 斗子型打平优先级（完美>整合>认知，老师规则 2025-06）
  ...
  function classifyMain(fingers) {
    ...
    // 步骤1：任意一指为反箕(Rl，字母l) → 逆思型
    // 注：此处"R1"是步骤编号，≠ 手指位置键fingers['R1']（右拇指）
  ...
  // ── 速测专用: 纯纹型判定 ────────────────────────────────────────────
  /**
   * classifyBySymbolsOnly(thumbs, allSymbols)
   *   → { mainType, key, tagline, perFinger, 叠加特质, 速测声明 }
   *
   * 速测链路专用 — 只读纹型符号，绝不读取任何 TRC / ATD 数值。
   * 适用场景: fingerprint-v2-wizard (用户自勾纹型, 无数值数据)
   * 完整版链路 (有 TRC 数值) 请使用 classify()，见 report-upload.html。
   *
   * thumbs:     { right: 'Ws', left: 'Lu' }   右手/左手拇指符号（兜底用）
   * allSymbols: { R1:'Ws', R2:'Lu', ..., L5:'Lu' }  十指符号 (无需 trc 字段)
   *
   * ⚠️  速测屏蔽：差值>3 / M型 / RULE-F04 / 任何 TRC 数值
   * "按主导拇指(TRC高者)决定" → 一律改为固定优先级 完美>整合>认知
   *
   * 符号→类型映射严格复用外层 LBL 表（已 1442 份真实报告验证），
   * 不在本函数内重新定义，防止与 classify() 产生不一致。
   *
   * Wi (内破斗) = 认知（不是整合）。
   * Rpe/Rwl/Wsr 带 +R 叠加，但不触发逆思型主类型；只有 Rl 才触发逆思型。
   */
  function classifyBySymbolsOnly(thumbs, allSymbols) {
    const syms = Object.values(allSymbols);

    // ── 每指精确输出（符号 + 类型标签，不塌缩到大类） ─────────────────
    const perFinger = {};
    const plusRFingers = [];  // Rpe/Rwl/Wsr 叠加 +R，不触发逆思型主类型
    for (const [pos, sym] of Object.entries(allSymbols)) {
      const label = _label(sym);   // 复用外层 LBL，完整且已验证
      perFinger[pos] = { sym, label };
      if (REVERSE.has(sym) && sym !== 'Rl') plusRFingers.push(pos);
    }

    // 拇指符号/标签（优先从 allSymbols 取，thumbs 参数作为兜底）
    const ltSym = allSymbols['L1'] || (thumbs && thumbs.left)  || 'Ws';
    const rtSym = allSymbols['R1'] || (thumbs && thumbs.right) || 'Ws';
    const ltLbl = _label(ltSym);
    const rtLbl = _label(rtSym);
    const ltFam = FAM[ltLbl] || '箕';
    const rtFam = FAM[rtLbl] || '箕';

    // 速测固定优先级：完美>整合>认知（替代"TRC高者主导拇指"规则；老师规则 2025-06）
    function _wDom(a, b) {
      return (SUBPRI[a] ?? 2) <= (SUBPRI[b] ?? 2) ? a : b;
    }

    // 结果包装器（统一附加 perFinger、叠加特质、速测声明）
    function _out(mainType, key, tagline, rlFingers) {
      const rFingers = rlFingers ? [...rlFingers, ...plusRFingers] : plusRFingers;
      return {
        mainType,
        key,
        tagline,
        perFinger,
        叠加特质: {
          M型: false,            // 速测无 TRC，无法计算 M 型
          逆向思维R: rFingers.length > 0,
          R手指: rFingers,
        },
        速测声明: '速测版 — 主类型已确定；天赋强弱/五功能区/精确通道占比需上传总表解锁',
      };
    }

    // ── STEP 1: 任意指为 Rl(反箕) → 逆思型 ──────────────────────────
    // ⚠️  只有 Rl 触发逆思型主类型；Rpe/Rwl/Wsr 只叠加 +R，主类型不变
    if (syms.some(s => s === 'Rl')) {
      const rlFingers = Object.entries(allSymbols)
        .filter(([, s]) => s === 'Rl').map(([p]) => p);
      return _out('逆思型', 'rl', '逆向思考 · 创意非凡 · 与众不同 · 挑战常规', rlFingers);
    }

    // 统计各纹型数量（全部经由外层 _label，与 classify() 完全一致）
    const isDou = s => ['认知', '整合', '完美'].includes(_label(s));
    const isArc = s => _label(s) === '开放';
    const totalDou = syms.filter(isDou).length;
    const totalArc = syms.filter(isArc).length;
    const totalLu  = syms.filter(s => s === 'Lu').length; // §三明确只认正箕 Lu

    // ── STEP 2: 十指全斗 → 超级认知 A/B/C ───────────────────────────
    // 速测无 TRC → 固定优先级 完美>整合>认知 决定子型（替代"主导拇指 TRC"规则）
    if (totalDou === 10) {
      const dom = _wDom(ltLbl, rtLbl);
      if (dom === '整合') return _out('超级认知型B', 'super_w_b', '领袖气质 · 全局整合 · 多维并行 · 跨界高手');
      if (dom === '完美') return _out('超级认知型C', 'super_w_c', '领袖气质 · 极致审美 · 完美追求 · 卓越标准');
      return _out('超级认知型A', 'super_w_a', '天生领袖 · 主见极强 · 目标必达 · 王者之气');
    }

    // ── STEP 3: 弧(X+Xn) ≥ 6 → 开放型 ──────────────────────────────
    if (totalArc >= 6) return _out('开放型', 'x', '海绵学习 · 踏实肯干 · 简单极致 · 开放接纳');

    // ── STEP 4: 十指全正箕(Lu) → 超级模仿型 ─────────────────────────
    if (totalLu === 10) return _out('超级模仿型', 'super_l', '敞开接纳 · 大公无私 · 奉献型 · 共情极强');

    // ── STEP 3b: 食指有弧(总弧不足6) → 主导拇指标签兼开放型 ──────────
    const idxR = allSymbols['R2'] || '', idxL = allSymbols['L2'] || '';
    if (isArc(idxR) || isArc(idxL)) {
      let domLbl;
      if (ltFam !== rtFam) {
        domLbl = FAMRANK[ltFam] <= FAMRANK[rtFam] ? ltLbl : rtLbl; // 高家族主导
      } else if (ltLbl === rtLbl) {
        domLbl = ltLbl;
      } else {
        domLbl = _wDom(ltLbl, rtLbl); // 同斗不同子型 → 固定优先级
      }
      // 主导也是开放（两拇均弧但总弧 < 6），fallthrough → STEP 5 输出开放型
      if (domLbl !== '开放') {
        return _out(`${domLbl}兼开放型`, 'combo_open', `${domLbl}主导 · 开放加持 · 吸收力强 · 多元适应`);
      }
    }

    // ── STEP 5: 看两拇指 ─────────────────────────────────────────────
    const SINGLE = {
      '认知': ['认知型',   'w',         '独立思考 · 主见权威 · 探索求知 · 逻辑构建'],
      '整合': ['整合型',   'wc',        '双核思维 · 多维整合 · 灵活高手 · 系统设计'],
      '完美': ['完美型',   'perfect_w', '极致标准 · 审美卓越 · 追求完美 · 品质至上'],
      '模仿': ['模仿型',   'l',         '善于模仿 · 适应力强 · 服务型 · 精准复现'],
      '开放': ['开放型',   'x',         '海绵学习 · 踏实肯干 · 简单极致 · 开放接纳'],
    };

    // (a) 两拇同标签 → 单一型
    if (ltLbl === rtLbl) {
      const [mt, k, tl] = SINGLE[ltLbl] || SINGLE['认知'];
      return _out(mt, k, tl);
    }

    // (b) 两拇不同家族 → 兼型（高家族在前：斗>箕>弧）
    if (ltFam !== rtFam) {
      const [a, b] = FAMRANK[ltFam] <= FAMRANK[rtFam] ? [ltLbl, rtLbl] : [rtLbl, ltLbl];
      const comboKey = [a, b].includes('开放') ? 'combo_open'
                     : [a, b].includes('模仿') ? 'combo_w_l'
                     : 'combo_w';
      return _out(`${a}兼${b}型`, comboKey, `${a}主导 · ${b}加持 · 多元天赋`);
    }

    // (c) 两拇同斗家族、不同子型 → 固定优先级取单一型（速测不兼）
    // 完美>整合>认知，给出确定答案（老师规则 2025-06）
    const dom = _wDom(ltLbl, rtLbl);
    const [mt, k, tl] = SINGLE[dom] || SINGLE['认知'];
    return _out(mt, k, tl);
  }
