-- AIPIWEN V3a Phase C1-D trusted phone identity and atomic registration.
--
-- Supabase Auth is the only identity source. Browser clients cannot write the
-- three pending-application tables directly; they submit through one
-- authenticated SECURITY DEFINER function that derives phone/email from
-- auth.users and creates all three records in one transaction.
...
create function public.v3a_submit_pending_application(
  p_display_name text,
  p_city text,
...
begin
  ...
  if v_practitioner_type not in (
    'independent', 'organization', 'agent', 'center', 'other'
  ) then
  ...
  v_jwt_phone := nullif(btrim(coalesce(v_jwt ->> 'phone', '')), '');
  ...
  if nullif(v_auth ->> 'phone_confirmed_at', '') is not null then
    ...
    if v_auth_phone !~ '^[+]861[3-9][0-9]{9}$' then
    ...
    if v_jwt_phone is not null then
      raise exception using errcode = '22023', message = 'AUTH_PHONE_NOT_VERIFIED';
    end if;
    v_phone := null;
  end if;
  ...
  if found then
    ...
    if v_user.status = 'pending' then
      ...
      if v_user.display_name is distinct from v_display_name
        ...
        or v_review.invite_code is distinct from v_invite_code then
...
comment on function public.v3a_submit_pending_application(
  ...
  'Creates one pending users/profile/application set atomically from the current verified Supabase Auth identity; identity fields are never client supplied.';
