-- AIPIWEN advisor interpretation V3: 16 structured modules with resumable generation.
-- Review artifact only. Execute separately in Preview before deploying V3 code.
...
begin
  if to_regclass('public.advisor_reports') is null
    ...
    raise exception 'MIGRATION_030_REQUIRES_MIGRATION_029';
  end if;
  ...
  if exists (
    ...
    where r.interpretation_data is not null
      and not (
        ...
        and r.interpretation_data ->> 'version' = '1'
    ...
    raise exception 'MIGRATION_030_UNEXPECTED_EXISTING_INTERPRETATION_DATA';
  end if;
end
$$;
          ...
          or (
            ...
            and (
              (
                ...
                and jsonb_array_length(interpretation_data -> 'steps') in (2, 4, 6, 8, 10, 12, 14)
              )
              or (
...
comment on column public.advisor_reports.interpretation_data is
  'Latest advisor interpretation plan. V1 stores legacy 8-step plans; V3 stores resumable 16-module plans.';
...
begin
  ...
  if not (
    ...
    or (
      v_version = '3'
      and (
        (v_status = 'generating' and v_step_count in (2, 4, 6, 8, 10, 12, 14))
...
begin
  if not exists (
    ...
    raise exception 'MIGRATION_030_POSTFLIGHT_CONSTRAINT_FAILED';
  end if;
  if not has_function_privilege(
    ...
    raise exception 'MIGRATION_030_POSTFLIGHT_RPC_PRIVILEGES_FAILED';
  end if;
  if has_table_privilege('authenticated', 'public.advisor_reports', 'INSERT')
    ...
    raise exception 'MIGRATION_030_POSTFLIGHT_TABLE_PRIVILEGES_FAILED';
  end if;
end
$$;
