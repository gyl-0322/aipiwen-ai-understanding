-- AIPIWEN advisor interpretation V3: support detailed 16-module payloads up to 128KB.
-- Execute in Preview only after migration 031.
...
begin
  if to_regclass('public.advisor_reports') is null
    ...
    raise exception 'MIGRATION_032_REQUIRES_MIGRATION_031';
  end if;
  ...
  if exists (
    ...
    where r.interpretation_data is not null
      and not (
        ...
        and octet_length(r.interpretation_data::text) <= 131072
        and (
          (
    ...
    raise exception 'MIGRATION_032_UNEXPECTED_EXISTING_INTERPRETATION_DATA';
  end if;
end
$$;
  ...
  add constraint advisor_reports_interpretation_data_check
    check (
      ...
      or (
        ...
        and octet_length(interpretation_data::text) <= 131072
        and (
          (
...
begin
  ...
  if jsonb_typeof(p_interpretation_data) is distinct from 'object'
     ...
     or octet_length(p_interpretation_data::text) > 131072 then
...
begin
  if not exists (
    ...
    raise exception 'MIGRATION_032_POSTFLIGHT_CONSTRAINT_FAILED';
  end if;
  if not has_function_privilege(
    ...
    raise exception 'MIGRATION_032_POSTFLIGHT_RPC_PRIVILEGES_FAILED';
  end if;
  if has_table_privilege('authenticated', 'public.advisor_reports', 'INSERT')
    ...
    raise exception 'MIGRATION_032_POSTFLIGHT_TABLE_PRIVILEGES_FAILED';
  end if;
end
$$;
