...
🧩 交付增强（各1条）
  通道比喻：针对主通道给1个生活比喻 + ▸ 1条今天能用的陪伴动作；
  现实锚点：如数据与家庭/成长背景吻合，明确点出（信任锚点）；
...
🧩 交付增强（各1条）
  通道比喻：针对主学习通道给1个生活比喻 + ▸ 1条家长今天能用的动作；
  现实锚点：如数据与孩子现实（家庭/学校/成长环境）吻合，明确点出（信任锚点）；
...
🧩 交付增强（各1条）
  通道比喻：针对主通道给1个比喻 + ▸ 1条高中学习能用的动作；
  现实锚点：如数据与当前学习状态/家庭背景吻合，明确点出（信任锚点）；
...
🧩 交付增强（各1条）
  通道比喻：针对主学习通道给1个生活比喻 + ▸ 1条落地动作；
  现实锚点：如数据与用户实际身份/职业/背景吻合，明确点出（信任锚点）；
...
const REPORT_STAGE_STYLE = {
  preschool: `【年龄段写作策略：0–6 学龄前 · 家长读】
- 用"这孩子/她/他"，不要对孩子本人说教。
- 场景必须落在吃饭、睡觉、情绪、探索、规则、兄弟姐妹/隔代养育。
- 必给模块要让家长知道：什么是天赋早期信号，今天怎么陪，哪些做法会把孩子推远。`,
  school: `【年龄段写作策略：7–12 小学 · 家长读】
- 用"这孩子/她/他"，站在帮孩子辩护、帮家长理解的角度。
- 场景必须落在作业、课堂、老师、同伴、兴趣班、亲子冲突。
- 必给模块要讲清：怎么学最省力，家长怎么配合，哪些科目/活动顺势，哪些地方别拿名次苛责。`,
  junior_teen: `【年龄段写作策略：13–15 初中 · 家长/本人双视角】
- 既让家长看懂，也让孩子读了不反感；少用"你家孩子不听话"，多翻译边界、自我意识和压力。
- 场景必须落在自驱力、情绪波动、亲子沟通、同伴关系、学习方法。
- 必给模块要讲清：不是叛逆本身，而是正在长出自己的判断和边界。`,
  senior_teen: `【年龄段写作策略：16–18 高中 · 家长/本人双视角】
- 语言中性，兼顾本人会读；不要幼儿化，也不要制造升学焦虑。
- 场景必须落在选科、专业方向、压力承载、学习效率、与父母沟通。
- 必给模块要讲清：如何用天赋结构做选择，哪些压力来自不适配，下一步怎么调。`,
  young_adult: `【年龄段写作策略：19–25 大学·初职 · 本人读】
- 直接对"你"说话，聚焦专业、职业、关系、自我认同和内耗。
- 场景必须落在考研就业、行业选择、团队角色、恋爱/父母关系、拖延与自信。
- 必给模块要讲清：你怎么使用自己，哪些赛道顺，哪些不是你的主场。`,
  adult: `【年龄段写作策略：25–40 职业发展 · 本人读】
- 直接对"你"说话，成熟、温和、务实，不要鸡汤。
- 场景必须落在职业瓶颈、创业/打工、婚姻沟通、育儿、团队角色、精力分配。
- 必给模块要讲清：优势如何在现实工作和关系里兑现，卡点如何调整。`,
  mature_adult: `【年龄段写作策略：40+ 成熟转型 · 本人读】
...
这是一份用户可能愿意付费保存、转发、继续深聊的完整报告，不是速测摘要。必须有"分量"，但每句话都要有信息增量。

1. 天赋底色：必须 3 段。
   - 第1段先看见人：把最容易被误解的行为/气质翻译出来，让用户觉得"这说的是我/我家孩子"。
   - 第2段叠加画像：主类型 × 学习通道 × ATD × 行为模式 × 1–2个最突出的功能区，揉成一个活人，不逐条念数据。
   - 第3段给定位：这个人放在哪种环境最顺，放在哪种环境最耗，最后用一句定心气的话收住。

2. 每个必给模块：至少 3 段 + 1 个明天能做的小事。
   - 必须出现具体生活场景，不要只写抽象结论。
   - 模块2负责"阶段刚需"：学前讲启蒙，小学讲学科/兴趣，中学讲选科升学，青年讲职业起步，成人讲职业优势，40+讲二次发展。
   - 模块3负责"怎么相处/怎么自我调整"，不能和模块2重复。

3. 每个可选问题：严格四段式，且每段写够。
   - ①为什么：2–3句，必须拴到具体类型/通道/ATD/某个脑区，不许泛泛心理分析。
   - ②怎么办：给2–3个可执行动作，每个动作说明怎么做、适合这个人的原因。
   - ③未来趋势：把困扰翻译成底色和长期价值，讲到用户心里一暖；不预测命运。
   - ④还想深聊：一句话，指出最值得继续追问的具体方向。

4. 语言质量红线：
   - 正文不要裸露 TRC/ATD/百分比/高于均值几分；数据只作为内部依据，输出要翻成人话。
   - 禁止"弱/差/不行/垫底"，改成"不是主场/需要换方法/别用这把尺子苛责"。
   - 禁止统一模板套话；每个板块至少出现一个与年龄段有关的真实场景。
   - 不诊断、不算命、不恐吓、不比较别人。`;
...
- 勾选问题四段式：每段写实写满——"怎么办"给2–3个具体动作说清怎么做，"未来趋势"把那束"光"讲到位。
...
【交付增强写法——兴趣班/职业板块重点运用】
- 通道比喻法：每讲一个核心特质，配一个生活比喻（让用户"被说中"），比喻后接 ▸ 落地动作。常用比喻：听觉型=怕唠叨（告知→提醒→第三次必烦，重复别超两次；讨论学习最快）；视觉型=外貌协会（靠预习、看表情穿着）；体觉型=考拉（粘熟人、给合法活动）；TRC=牛顿摆铁球（承载有限，满了就弹掉一个）。
- 学习通道三维：写学习通道板块时必须讲三件：①怎么学最快 ②怎么舒压 ③怎么沟通最有效（不能只讲一个维度）。
- 叠加解读：综合/交叉画像段不要罗列数据，要层层叠加（类型→通道→ATD→行为→组合落点），"整个人一下就立起来"；点出"卡点/矛盾"（性格与通道/ATD之间的张力，往往最有价值）。
- 现实验证：数据与用户现实（职业/人生阶段/身份）吻合时，主动点出（"这不是纸上判断，是已被你的人生验证的"）——最强信任锚点。
- 给信心：精神功能偏低者容易低估自己，主动点出其他优势，不让TA自我否定。
...
①为什么：（在「${requiredModules[1]}」结论基础上，点明此决策的关键数据依据，2-3句，翻成人话，不裸露数值）
②怎么办：（针对"${issue}"给出3条具体可选路径，每条说明适合原因和第一步怎么做）
③未来趋势：（这个方向的长期价值，2-3句，把卡点翻成可使用的优势）
④还想深聊：（一句话邀请，指向最有价值的下一个追问）`;
        }
        return `===issue:${issue}===\n①为什么：（基于具体手指/类型/通道/ATD的根本原因，2-3句，先翻译行为背后的需要）\n②怎么办：（给2-3个明天能做的具体动作，每个动作说明怎么做、为什么适合这个人）\n③未来趋势：（2-3句，把当前困扰翻成底色和长期价值，讲出那束光）\n④还想深聊：（一句话邀请，指出最有价值的追问方向）`;
      }).join('\n\n')
    : '';
...
总TRC：${fp['总TRC']}
${兴趣班提示}
${REPORT_STAGE_STYLE[tier] || REPORT_STAGE_STYLE.adult}
${REPORT_DEPTH_CONTRACT}
【需要生成的板块】（按顺序）
${allModules.map((m,i)=>`${i+1}. ${m}`).join('\n')}

【必给板块格式】每个必给板块用 ===板块名=== 开头。天赋底色必须3段；其他必给板块至少3段，并在末尾给一个“明天可以试的一件事”。具体、有生活场景、有比喻，但不要把数据清单直接端给用户。
⚠️ 同源一致（00i 规则2）：凡涉及职业/能力/兴趣班的必给板块（如「${requiredModules[1]}」），同样严格基于上方 RULE-F04 已修正判定展开，不另行重算高低；后续可选延伸问题将在此结论基础上深化，必须保持一致。
⚠️ 正文零裸数字：TRC/ATD/百分比/高于平均几分只能作为内部依据，输出时翻译成“最顺手的通道/承载节奏/不是主场”等人话。
...
function parseSections(raw, requiredModules, selectedIssues) {
  ...
  while ((m = blockRegex.exec(raw)) !== null) {
    ...
    if (isIssue) {
      ...
      const whyM    = body.match(/①\s*为什么[^：:\n]*[：:]([\s\S]*?)(?=②|$)/i);
      const howM    = body.match(/②\s*怎么办[^：:\n]*[：:]([\s\S]*?)(?=③|$)/i);
      const futureM = body.match(/③\s*未来趋势[^：:\n]*[：:]([\s\S]*?)(?=④|$)/i);
      const ctaM    = body.match(/④\s*还想(?:聊|深聊)[^：:\n]*[：:]?([\s\S]*?)$/i);
...
module.exports = async function handler(req, res) {
  ...
  const { engineResult, age, name, selectedIssues: rawSelectedIssues = [], refToken = null, fingers = null } = payload;
  if (!engineResult) return res.status(400).json({ ok:false, error:'缺少 engineResult' });
  const selectedIssues = Array.isArray(rawSelectedIssues) ? rawSelectedIssues.slice(0, 3) : [];
