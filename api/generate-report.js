...
- 固定骨架不是目录页：每个必给模块都要认真展开，不能每段只写一句话；宁可少用空泛形容词，也要把"这个指标在生活里怎么出现、为什么和我有关、明天怎么用"讲透。前台不要露出"是什么/对你意味着什么/怎么应用"这种模板标题。
...
- 全报告最低信息量：每个固定模块总量建议350-650字；性格类型和五大功能模块建议600-900字。每个固定模块至少2-4段，必须包含"指标解释 + 当前数据/个人均值关系 + 现实行为画面 + 优势与限制 + 可执行建议"中的至少3类信息。
...
- 每个必给板块内部必须覆盖三层信息：指标/系统在生活里是什么、放到当前用户身上怎么理解、可以怎样应用到学习/行为/沟通。前台表达要自然，不要把小标题写成"是什么/对当前用户意味着什么/怎么应用"。
...
每个固定模块必须自然覆盖三层内容，但不要在前台露出模板标题：
第一层：用用户听得懂的话解释这个指标/系统在生活里通常怎么出现，可以举生活例子；不能只给定义。
第二层：结合当前用户数据，和个人均值或区间对比，讲高/低/均衡的优势、代价和理解方式，禁止只讲理论；必须落到当前年龄对应对象的行为画面。
第三层：讲这些特征如何应用到学习、行为、沟通；如何理解、接纳、和解、和谐共生；必须给2-4个具体可做动作或观察点。
...
function coreModuleFallback(title, engineResult, tier = 'adult', fingers = null) {
  ...
  const functionFallback = (cfg) => {
    ...
    return [
      `这个板块不把两根手指简单相加来判断强弱，而是分别看${cfg.rightName}和${cfg.leftName}。${cfg.rightName}代表${cfg.rightRole}；${cfg.leftName}代表${cfg.leftRole}。两边合在一起，才构成这一项功能在生活里的完整样子。`,
      `放到当前数据里，${cfg.rightName}数值为${rightValue}，${fingerState(cfg.rightPos)}；${cfg.leftName}数值为${leftValue}，${fingerState(cfg.leftPos)}。${pairState(cfg.rightPos, cfg.leftPos, cfg.rightName, cfg.leftName)} 当某一侧偏高时，${cfg.high}；当某一侧偏低时，${cfg.low}。所以这里看的不是“这个功能好不好”，而是它从哪一侧更容易被唤起、哪一侧更需要环境支持。`,
      `实际使用时，${cfg.apply} 如果两侧都高，要给空间和出口，避免能量被压成急躁；如果两侧都低，要先搭脚手架，不要用“你怎么做不到”去刺激。真正有效的用法，是把高的一侧当入口，把低的一侧当需要慢慢补足的节奏。`
    ].join('\n\n');
  };

  const lines = {
    '严正申明四原则': [
      `在正式看报告之前，先把一件事说清楚：这份报告不是给${audience.subject}下结论，也不是把人分成好坏高低。它更像一张理解自己的地图，帮${audience.readerAndSubject}看见哪些地方更省力、哪些地方需要方法，哪些反应是天生更容易出现的倾向，哪些又会被${audience.context}影响。数值只是线索，不是判决；报告的价值不在于“说中了多少”，而在于它能不能帮${audience.readerAndSubject}更温和、更具体地理解自己。`,
      `接下来你会看到TRC、ATD、左右脑、性格类型、学习通道、行为模式和五大功能区。看到高的地方，不要只理解成“优势”，它也可能因为用得太顺而变成急、满、停不下来；看到低的地方，也不要理解成“短板”，它可能只是说明这个入口更需要节奏、提醒、练习或安全感。所有数值都只和自己的个人均值比，不拿你和别人比；同样的先天线索，放在不同家庭、不同学校、不同成长经历里，也会长出完全不同的样子。`,
      `读这份报告时，可以带着四个原则往下看：第一，数值没有好坏，只看它在生活里怎么被使用；第二，报告不预测未来，也不替你做重大决定；第三，高低都回到自己的均值和真实表现里理解；第四，不用任何一个类型或指标给人贴标签。最好的读法不是急着判断“准不准”，而是拿它去对照真实场景：最近哪里更顺，哪里总卡住，哪里一被催就乱，哪里一被看见就能发挥。这样，这份报告才会从一张数据表，变成一份可以帮助学习、沟通和自我接纳的说明书。`
    ],
    'TRC（认知结构）': [
