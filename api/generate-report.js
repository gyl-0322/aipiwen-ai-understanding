...
②对你意味着什么：结合当前用户数据，和个人均值或区间对比，讲高/低/均衡的优势、代价和理解方式，禁止只讲理论；必须落到"这个孩子/这个人"的行为画面。
...
- ===性格类型（核心行为外显模块）=== 必须最详细，是整份报告的"用户共鸣页"：必须写清类型定义、典型行为画面、学习中的表现、人际/亲子沟通中的表现、压力下容易被误解的地方、优势如何被看见和使用。不要只讲理论，要让家长/本人读完觉得"这说的就是我/我家孩子"。
...
function coreModuleFallback(title, engineResult) {
  ...
  const lines = {
    '严正申明四原则': [
      `①是什么：在正式看报告之前，先把一件事说清楚：这份报告不是给你或孩子下结论，也不是把人分成好坏高低。它更像一张理解自己的地图，帮你看见哪些地方更省力、哪些地方需要方法，哪些反应是天生更容易出现的倾向，哪些又会被家庭节奏、学习环境、年龄阶段和最近压力影响。数值只是线索，不是判决；报告的价值不在于“说中了多少”，而在于它能不能帮你更温和、更具体地理解自己。`,
    ...
    'TRC（认知结构）': [
      ...
      `②对当前用户意味着什么：当前总TRC为${total}，个人均值为${avg}。高于个人均值的区域更像顺手工具，低于个人均值的区域更像需要节奏和方法支持的地方；理解它，是为了知道哪些事可以顺势用力，哪些事不要硬扛。比如学习时，容量强的部分可以承担理解和整合，容量弱的部分就不要用连续轰炸式练习硬推。如果一个孩子看起来“听懂了但做不出来”，有时不是态度问题，而是信息量、步骤量和输出要求同时压上来，超过了当下最舒服的承接方式。`,
    ...
    'ATD（感受/反应节奏）': [
      ...
      `②对你意味着什么：当前ATD为${atd['值'] || '未识别'}，处于${atd['分区'] || '待确认'}。节奏偏快时容易反应灵敏、先感觉到变化，也可能急、抢答、被一句话点燃；节奏偏慢时更稳、更能沉住气，也可能被误会为拖、慢热、不积极。放在孩子身上，ATD常常会影响他被催时的反应：有的孩子一催就炸，有的孩子越催越慢，还有的孩子需要先把规则听完整才愿意动。`,
    ...
    '左右脑（信息处理风格）': [
      ...
      `③怎么应用：学习上，可以把抽象内容转成步骤或图像：偏左的孩子先给公式、框架、清单；偏右的孩子先给故事、图像、例子，再回到规则。沟通上，先用对方听得懂的方式进入，再谈要求；比如先画出任务流程，或者先说明这件事和他有什么关系。做选择时，也允许自己既看事实，也看身体和情绪给出的提醒，这样判断会更完整。`
    ],
    '性格类型（核心行为外显模块）': [
    ...
    '学习通道（学习输入系统）': [
      `①是什么：学习通道指信息最容易从哪里进入大脑，是听、看、动手、画面、语言或身体体验等输入方式的偏好。它不是判断一个人只能用哪一种方式学习，而是在提醒我们：同样一段内容，有的人听一遍就抓住重点，有的人看图更快，有的人必须动手做过才真正明白。通道对了，学习会省力；通道不对，孩子可能不是不会，而是入口没打开。`,
    ...
    '行为模式（行为解释系统）': [
      ...
      `②对你意味着什么：当前行为模式为${behav['结论'] || '待确认'}。这说明某些行为不是单纯听不听话、努不努力，而可能是启动方式、压力承接方式或反馈节奏不匹配。当一个孩子反复在同一类场景里卡住，我们更需要问：他是没理解、没开始、没坚持，还是被情绪打断？如果只评价“懒、不认真、脾气大”，就容易错过真正能调整的入口。`,
      `③怎么应用：遇到卡住时，先看行为背后的需求，再给方法。学习上减少空泛催促，沟通上把要求说清楚、把步骤拆开、把反馈放近，行为才更容易回到合作。可以先观察三个点：这个行为通常发生在什么任务前；大人说了什么之后加重；孩子完成哪一步后会明显放松。找到这些线索，再给支持，效果会比单纯讲道理稳定得多。`
    ],
    '精神功能（拇指系统）': [
