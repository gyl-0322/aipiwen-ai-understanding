...
🧩 三家精华（各1条）
  艾尔发：针对主通道给1个生活比喻 + ▸ 1条今天能用的陪伴动作；
  359叠环境：如数据与家庭/成长背景吻合，明确点出（信任锚点）；
...
🧩 三家精华（各1条）
  艾尔发：针对主学习通道给1个生活比喻 + ▸ 1条家长今天能用的动作；
  359叠环境：如数据与孩子现实（家庭/学校/成长环境）吻合，明确点出（信任锚点）；
...
🧩 三家精华（各1条）
  艾尔发：针对主通道给1个比喻 + ▸ 1条高中学习能用的动作；
  359叠环境：如数据与当前学习状态/家庭背景吻合，明确点出（信任锚点）；
...
🧩 三家精华（各1条）
  艾尔发：针对主学习通道给1个生活比喻 + ▸ 1条落地动作；
  359叠环境：如数据与用户实际身份/职业/背景吻合，明确点出（信任锚点）；
...
【三家精华整合（来源：00g）——兴趣班/职业板块重点运用】
- 艾尔发·比喻法：每讲一个核心特质，配一个生活比喻（让用户"被说中"），比喻后接 ▸ 落地动作。常用比喻：听觉型=怕唠叨（告知→提醒→第三次必烦，重复别超两次；讨论学习最快）；视觉型=外貌协会（靠预习、看表情穿着）；体觉型=考拉（粘熟人、给合法活动）；TRC=牛顿摆铁球（承载有限，满了就弹掉一个）。
- 艾尔发·学习三维：写学习通道板块时必须讲三件：①怎么学最快 ②怎么舒压 ③怎么沟通最有效（不能只讲一个维度）。
- 359·叠加解读：综合/交叉画像段不要罗列数据，要层层叠加（类型→通道→ATD→行为→组合落点），"整个人一下就立起来"；点出"卡点/矛盾"（性格与通道/ATD之间的张力，往往最有价值）。
- 359·解读生命：数据与用户现实（职业/人生阶段/身份）吻合时，主动点出（"这不是纸上判断，是已被你的人生验证的"）——最强信任锚点。
- 给信心（艾尔发）：精神功能偏低者容易低估自己，主动点出其他优势，不让TA自我否定。
