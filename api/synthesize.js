...
【重要格式要求】禁止用"收到""好的""当然""明白""我来帮你"等开场白。直接输出结构化分析。`;
  ...
  const userPrompt = `以下是同一个用户在不同关系场景下积累的行为洞察记录，请从家庭系统视角进行综合分析：

${contextSections}

请按以下结构输出（语言要有洞察力和温度，合计300字以内）：

**系统主题**（1-2句话，说出这个人在所有关系中最核心的那个模式或张力——要让用户看到就觉得"对，就是这个"）

**跨场景联系**（2-3条，说明这些场景的行为如何相互影响和强化——要有因果逻辑，不是简单平行罗列）

**优先改变建议**（3条，按杠杆大小排序，每条说明：在哪里改变 + 这个改变为什么能同时影响多段关系 + 一个具体的下一步行动）`;
  ...
  const connectionsMatch = reply.match(/\*\*跨场景联系\*\*[：:]?\s*([\s\S]*?)(?=\*\*优先改变建议\*\*|$)/);
