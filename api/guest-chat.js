...
const MATERIAL_OBSERVATION_RULE = `
【已有材料辅助观察边界】
- TRC、ATD、学习通道、功能区和类型名称，只能在用户上传材料明确出现时按原始数据记录，并注明材料来源。
- 这些字段只用于整理可继续观察和沟通的线索，不代表人格、能力、学业或未来发展的确定结论。
- 不得根据日常对话反推数值、手指、脑区或类型；材料缺失时明确写“未提供”，不得猜测或补全。
- 建议必须落到可持续观察的真实场景，并提醒用户结合孩子后续表现与人工沟通进一步核对。
`;
...
- 全程：每次回复结尾，用一句话自然地邀请用户往下一步走`;
  ...
  // 仅把证据边界拼入模型提示；旧类型知识源保留在知识库文件中，不作为当前品牌主表达。
  const behaviorOnlyEvidenceRule = isBehaviorOnly
    ...
    : '';
  const trcContent = isBehaviorOnly ? behaviorOnlyEvidenceRule : MATERIAL_OBSERVATION_RULE;
...
字数：500-600字`,
  ...
  const reportSummarySection = reportSummary
    ? `\n【已从报告提取的数据】${reportSummary.replace(/^AIPIWEN_DATA\|/, '').replace(/\|/g, ' · ')}\n`
    : '';
