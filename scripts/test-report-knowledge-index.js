...
const indexSource = fs.readFileSync(path.join(__dirname, '..', 'data', 'report-knowledge-index', 'report-knowledge-index-v1.json'), 'utf8');
const apiSource = fs.readFileSync(path.join(__dirname, '..', 'api', 'generate-report.js'), 'utf8');
...
assert(!indexSource.includes('/Users/'), 'runtime index must not contain local absolute paths');
assert(!indexSource.includes('offline_review_required_before_any_use'), 'unapproved expert-course candidate envelopes must not enter runtime index');
assert(apiSource.includes("require('../lib/report-knowledge-index')"), 'generate-report must load the curated runtime index helper');
assert(apiSource.includes('buildReportKnowledgePromptContext('), 'generate-report must build retrieval context before model generation');
assert(apiSource.includes('knowledge_context_skipped'), 'knowledge retrieval failure must keep an explicit non-blocking fallback');
assert(apiSource.includes('knowledgeContext\n    );'), 'generate-report must inject retrieval context into the report prompt');
...
const schoolDryRun = buildReportKnowledgeRetrievalDryRun({
  ageBand: '小学7-12岁',
  userIdentity: 'parent',
  reportSubject: 'child',
  selectedIssues: [
  ...
  reportModules: ['行为模式（行为解释系统）'],
});
assert(schoolDryRun.uniqueHitIds.includes('RKI-V1.3-SCHOOL-HOMEWORK'), 'school dry-run should hit school homework card');
assert(!schoolDryRun.uniqueHitIds.includes('RKI-V1.3-JUNIOR-BOUNDARY'), 'school dry-run must not retrieve junior boundary card');
assert(!schoolDryRun.uniqueHitIds.includes('RKI-V1-030'), 'school dry-run must not retrieve junior age-stage card');
...
assert(!riskDryRun.uniqueHitIds.includes('RKI-V1.3-SENIOR-EDUCATION'), 'rewrite-required education card must not enter automatic P0 retrieval');
assert(
...
assert.strictEqual(promptContext.mode, 'v1.5_prompt_context');
