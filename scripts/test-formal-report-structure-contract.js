...
assert(apiSource.includes('isRequiredModuleComplete(title, content)'), 'API 没有检查固定模块完整性');
...
const completeMental = [
  ...
  '右拇数值56，明显高于个人均值26.4，行动和目标更容易向外发起；左拇数值30，略高于个人均值26.4，内在管理和收尾也有可调用基础。',
  '右侧比左侧更高，右侧会先被调动。学习中可以先给挑战和责任，再把收尾标准说具体；沟通时观察孩子卡在理解、启动、坚持还是收尾。数值高低都不是好坏结论，需要结合年龄和现实场景连续观察。',
  '如果孩子愿意开始却常在最后一步松掉，可以把目标拆成启动、推进和收尾三个检查点。先让较强的开创入口带动行动，再用清单、时间边界和具体反馈支持管理入口。连续观察一到两周，根据真实变化调整，而不是只凭一次表现下结论。',
].join(' '.repeat(2));
assert(validate('精神功能（拇指系统）', completeMental), '完整的精神功能单指解释被误判为不完整');
assert(!validate('精神功能（拇指系统）', '右拇很高，左拇较低。'), '过短的精神功能内容没有被拦截');
assert(!validate('精神功能（拇指系统）', `${completeMental} 精神功能为86，明显高于个人均值26.4。`), '两指合计与单指均值的错误比较没有被拦截');
