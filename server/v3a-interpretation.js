...
function buildPrompt(report, client, input, stepIndexes = STEP_TITLES.map((_, index) => index)) {
  ...
  const user = [
    ...
    `本次必须且只能生成以下板块：\n${requestedSteps.join('\n')}`,
    `steps数组必须恰好包含${stepIndexes.length}个对象，不得增加、遗漏或合并板块。`,
    '每个板块必须包含stepIndex、title、why、say、ask、no、action、risk；stepIndex必须使用上方指定的0基序号，不得从1重新编号；title必须与指定板块一致。',
    '每个板块固定输出14条独立内容：why恰好2条、say恰好3条、ask恰好2条、no恰好2条、action恰好3条、risk恰好2条。why讲清依据和解读价值；say给可以直接照着讲的完整话术；ask用于核对生活场景和客户感受；no提示禁语和错误解释；action写清动作、观察点和复盘；risk说明资料边界、复核或转介条件。',
